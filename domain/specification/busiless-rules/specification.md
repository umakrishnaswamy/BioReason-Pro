...
temporal split artifact を基に、次の 2 系統の dataset を作る。
...
CoreWeave 上の実行では、任意の local directory を毎回手で渡すのではなく、repo 管理された registry を entry point にする。

固定ルール:

- data bundle registry は `configs/disease_benchmark/data_registry.json` を使う
- evaluation target registry は `configs/disease_benchmark/eval_target_registry.json` を使う
- data lineage は W&B Artifact reference を registry に保持する
- public checkpoint は Hugging Face source を registry に保持し、local に無ければ自動取得してよい
- private / internal checkpoint は W&B model artifact source を registry に保持する
- BLAST / Diamond と ESM 系単体は prediction artifact source として registry に保持する
