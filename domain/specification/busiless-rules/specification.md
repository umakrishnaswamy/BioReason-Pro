...
- `data/artifacts/benchmarks/214_221_225_228/temporal_split`
...
### 5.1 temporal split artifact build

temporal split artifact の生成スクリプトは `scripts/build_disease_temporal_split_artifact.py` を使う。

current implementation run:

```bash
python scripts/build_disease_temporal_split_artifact.py \
...
```bash
python scripts/build_disease_temporal_split_artifact.py \
  --output-dir data/artifacts/benchmarks/214_221_225_228/temporal_split \
...
- `BioReason-Pro SFT` と `BioReason-Pro RL` は public Hugging Face source から W&B model artifact に固定してよい
- `BioReason-Pro base` は private / local checkpoint を source にして W&B model artifact に固定する
- `BLAST / Diamond` と `ESM 系単体` は checkpoint ではなく prediction artifact として登録する

### 6.4 registry files

ここでいう registry は W&B Registry を指す。  
CoreWeave 上の実行では、任意の local directory を毎回手で渡すのではなく、repo 内に置く **W&B Registry path manifest** を entry point にする。
...
- W&B Registry path の貼り付け用 env template は `configs/disease_benchmark/wandb_registry_paths.env.example` を使う
- source 用 env template は `configs/disease_benchmark/wandb_asset_sources.env.example` を使う
- manifest に渡す registry path は browser URL ではなく `entity/project/artifact_name:alias` 形式の W&B Registry ref を使う
- data lineage は W&B Registry path を manifest に保持する
- public checkpoint の source 情報は manifest に保持し、一度 W&B artifact 化した後は registry ref を source-of-truth にする
- private / internal checkpoint は W&B model artifact の registry path を manifest に保持する
- BLAST / Diamond と ESM 系単体は prediction artifact の registry path として manifest に保持する
...
- `BLAST / Diamond` prediction artifact
- `ESM 系単体` prediction artifact
- `BioReason-Pro base`
- `train_sft` の出力
...
- `base-family`: `BLAST / Diamond`, `ESM 系単体`, `BioReason-Pro base`
- `tuned-family`: `train_sft`, `train_rl`
...
- prediction artifact baseline では reasoning 列を空文字列にしてよい
...
- base model は W&B Registry ref から local に materialize して使う
- 前半フェーズと後半フェーズの両方で、base model に含まれる projector / GO module 重みを warm-start に使ってよい
...
- base checkpoint の取得元は local path を手入力せず、W&B Registry ref を既定入力にする
